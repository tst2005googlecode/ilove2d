...
namespace sdl
{
	...
	bool Event::push(Message & message)
	{
		...
		return ok && (FSDL_PushEvent(&e) == 0);
	}
    

	bool Event::convert(FSDL_Event & e, Message & m)
	{
		switch(e.type)
		{
//		case SDL_KEYDOWN:
...
//			return keys.find(e.key.keysym.sym, m.keyboard.k);
		case FSDL_MOUSEBUTTONDOWN:
		case FSDL_MOUSEBUTTONUP:
			m.type = (e.type == FSDL_MOUSEBUTTONDOWN) ? Event::TYPE_MOUSE_PRESSED : Event::TYPE_MOUSE_RELEASED;
			m.mouse.x = e.mouse.x;
			m.mouse.y = e.mouse.y;
			return buttons.find(e.mouse.button, m.mouse.b);
...
//			return true;
	...
	bool Event::convert(Message & m, FSDL_Event & e)
	{
		switch(m.type)
		{
...
//			return keys.find(m.keyboard.k, e.key.keysym.sym);
		...
		case Event::TYPE_MOUSE_RELEASED:
			e.type = (m.type == Event::TYPE_MOUSE_PRESSED) ? FSDL_MOUSEBUTTONDOWN : FSDL_MOUSEBUTTONUP;
			e.mouse.x = m.mouse.x;
			e.mouse.y = m.mouse.y;
			return buttons.find(m.mouse.b, e.mouse.button);
...
/*
	...
	EnumMap<love::mouse::Mouse::Button, unsigned short, love::mouse::Mouse::BUTTON_MAX_ENUM>::Entry Event::buttonEntries[] = 
	{
		{ love::mouse::Mouse::BUTTON_LEFT, FSDL_BUTTON_LEFT},
		{ love::mouse::Mouse::BUTTON_MIDDLE, FSDL_BUTTON_MIDDLE},
		{ love::mouse::Mouse::BUTTON_RIGHT, FSDL_BUTTON_RIGHT},
		{ love::mouse::Mouse::BUTTON_WHEELUP, FSDL_BUTTON_WHEELUP},
		{ love::mouse::Mouse::BUTTON_WHEELDOWN, FSDL_BUTTON_WHEELDOWN},
		{ love::mouse::Mouse::BUTTON_X1, FSDL_BUTTON_X1},
		{ love::mouse::Mouse::BUTTON_X2, FSDL_BUTTON_X2},
	};

	EnumMap<love::mouse::Mouse::Button, unsigned short, love::mouse::Mouse::BUTTON_MAX_ENUM> Event::buttons(Event::buttonEntries, sizeof(Event::buttonEntries));
